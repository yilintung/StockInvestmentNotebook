...
class StockAnalysis :
    ...
    def _loading_price_data( self, stock_id) :
        ...
        if stock_id in df_stock_info.index:
            ...
            self._daily_price_df_len     = len(self._daily_price_df)
            self._debug_print('日Ｋ價格資料總數＝ {}'.format(self._daily_price_df_len))
            ...
            # TODO : 持續驗證中
            if weekly_price_df.empty :
                self._debug_print('錯誤：週Ｋ資料為空')
                return False
    ...
    def _k_line_pattern_recogntion( self) :
        # 當價格資料筆數不足時之處理程序
        # TODO : 持續驗證中
        if self._daily_price_df_len < 5 :
            description_str = '（價格資料筆數不足）'
    ...
    def _rank_evaluate( self) :
        ...
        # 當價格資料筆數不足時之處理程序
        # TODO : 持續驗證中
        if self._daily_price_df_len < 60 :
            description_str = '（價格資料筆數不足）'
            return description_str
        ...
        # 日Ｋ價格資料筆數不足時之處理程序
        if self._daily_price_df_len < 240 :
            description_str = description_str+ '長期位階（價格資料筆數不足）'
            return description_str
    ...
    def _price_volume_relationship( self) :
        ...
        # 當價格資料筆數不足時之處理程序
        # TODO : 持續驗證中
        if self._daily_price_df_len < 20 :
            description_str = description_str + '（價格資料筆數不足）'
            return description_str
    ...
    def _pattern_recognition( self) :
        ...
        # 當價格資料筆數不足時之處理程序
        # TODO : 持續驗證中
        if self._daily_price_df_len < 360 :
            description_str = '（價格資料筆數不足）'
            bottom_pattern_image = None
    ...
    def _moving_average_trend( self) :
        ...
        # 確認移動平均線方向：↑ : 'u' , ↓ : 'd' , ＝ : '='　, ？ : 'x'
        # TODO : 持續驗證中
        talib_daily_sma5_dir = 'x' if np.isnan(now_talib_daily_sma['SMA5']) or np.isnan(prev_talib_daily_sma['SMA5']) else 'u' if now_talib_daily_sma['SMA5'] > prev_talib_daily_sma['SMA5'] else 'd' if now_talib_daily_sma['SMA5'] < prev_talib_daily_sma['SMA5'] else '='
        talib_daily_sma10_dir = 'x' if np.isnan(now_talib_daily_sma['SMA10']) or np.isnan(prev_talib_daily_sma['SMA10']) else 'u' if now_talib_daily_sma['SMA10'] > prev_talib_daily_sma['SMA10'] else 'd' if now_talib_daily_sma['SMA10'] < prev_talib_daily_sma['SMA10'] else '='
        talib_daily_sma20_dir = 'x' if np.isnan(now_talib_daily_sma['SMA20'])  or np.isnan(prev_talib_daily_sma['SMA20']) else 'u' if now_talib_daily_sma['SMA20'] > prev_talib_daily_sma['SMA20'] else 'd' if now_talib_daily_sma['SMA20'] < prev_talib_daily_sma['SMA20'] else '='
        talib_daily_sma60_dir = 'x' if np.isnan(now_talib_daily_sma['SMA60']) or np.isnan(prev_talib_daily_sma['SMA60']) else 'u' if now_talib_daily_sma['SMA60'] > prev_talib_daily_sma['SMA60'] else 'd' if now_talib_daily_sma['SMA60'] < prev_talib_daily_sma['SMA60'] else '='
        talib_daily_sma120_dir = 'x' if np.isnan(now_talib_daily_sma['SMA120']) or np.isnan(prev_talib_daily_sma['SMA120']) else 'u' if now_talib_daily_sma['SMA120'] > prev_talib_daily_sma['SMA120'] else 'd' if now_talib_daily_sma['SMA120'] < prev_talib_daily_sma['SMA120'] else '='
        talib_daily_sma240_dir = 'x' if np.isnan(now_talib_daily_sma['SMA240']) or np.isnan(prev_talib_daily_sma['SMA240']) else 'u' if now_talib_daily_sma['SMA240'] > prev_talib_daily_sma['SMA240'] else 'd' if now_talib_daily_sma['SMA240'] < prev_talib_daily_sma['SMA240'] else '='
        # 短期趨勢
        if talib_daily_sma5_dir == 'x' or talib_daily_sma10_dir == 'x' :
            description_str = description_str + '（移動平均線資料筆數不足）'
        else :
        ...
        description_str = description_str + '；'
        # 中期趨勢
        if talib_daily_sma20_dir == 'x' or talib_daily_sma60_dir == 'x' :
            description_str = description_str + '（移動平均線資料筆數不足）'
        else :
        ...
        description_str = description_str + '；'
        # 長期趨勢
        if talib_daily_sma120_dir == 'x' or talib_daily_sma240_dir == 'x' :
            description_str = description_str + '（移動平均線資料筆數不足）'
        else :
    ...
    def _macd_long_short( self) :
        ...
        # TODO : 持續驗證中
        if np.isnan(curr_macd_dif) or np.isnan(curr_macd_macd) :
            description_str = '（ＭＡＣＤ指標資料筆數不足）'
            return description_str
    ...
    def _weekly_kd_cross( self) :
        ...
        sqlcmd                 = "SELECT * FROM WeeklyPrice WHERE StockID='{}' AND Date='{}' ".format(self._stock_id,weekly_end_date)
        df                     = pd.read_sql_query(sqlcmd, self._conn)
        if df.empty is True :
    ...
    def _overall_evaluation( self) :
        ...
        # TODO：更新至GPT-5.2，驗證Prompt v1.1中
        system_prompt = "你是一位具備專業技術分析能力的股市分析師，請依據使用者提供的技術分析圖表進行結構化技術分析；圖表包含：主圖K線（紅K=收高於開、黑K=收低於開），其價格資料於參考資料中分別對應Open（開盤價）、High（最高價）、Low（最低價）、Close（收盤價）；並搭配四條移動平均線（棕色=5日線，代表短期趨勢，在參考資料中對應SMA5；天藍色=10日線，代表短期趨勢，在參考資料中對應SMA10；紫色=20日線，代表中期趨勢，在參考資料中對應SMA20；橙色=60日線，代表中期趨勢，在參考資料中對應SMA60），子圖一為成交量柱（顏色與K線同步，在參考資料中對應Volume），子圖二為KD指標（紅線=K線，在參考資料中對應slowk、藍線=D線，在參考資料中對應slowd、綠色虛線標示超買>80與超賣<20），子圖三為MACD指標（紅線為DIF線，在參考資料中對應macd；藍線為MACD線，在參考資料中對應macdsignal；灰色柱狀體為OSC，在參考資料中對應macdhist）；在技術指標判讀時，若價格與KD或MACD出現背離，請將其視為動能變化的重要訊號；多頭背離為價格創新低但KD或MACD未創新低，空頭背離為價格創新高但KD或MACD未創新高；當訊號出現衝突時，判讀優先順序為：價格趨勢→均線排列→MACD動能→KD位置；KD處於超買或超賣區時不得單獨作為趨勢反轉依據，需結合價格與MACD動能判斷；MACD解讀時以DIF與MACD線的相對位置、方向與柱狀體變化判斷動能，避免使用快線或慢線等稱呼；分析時請僅針對圖表所呈現的當前狀態進行判讀與結論整理，參考資料僅用於輔助確認相對位置與方向，不得直接引用其數值、日期或欄位名稱於輸出中，不需解釋指標計算方式、背景原理或一般性教學說明，避免使用泛化、推論型或教科書式語句；若輸出內容中出現日期，請統一使用「MM月DD日」格式（例如2月10日），不得使用MM/DD、YYYY-MM-DD或其他數字斜線格式；在支撐與壓力判斷上，若可合理判斷請務必給出具體價格或點位，若無明確依據請直接說明「尚未形成」，避免模糊描述；請將價格趨勢、價量關係、支撐與壓力、KD指標解讀、MACD指標解讀、綜合評價分段呈現，且每一段落請控制在2至3句以內；請使用專業且通順的繁體中文回覆，凡涉及價格或成交量之數值須附單位（價格為元或點，成交量為張或億元），統一用詞：回調→回檔、止盈→停利、止損→停損，不需加入其他說明。"
